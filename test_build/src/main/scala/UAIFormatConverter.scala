...
val header = "MARKOV"
  ...
  def getUAIFactorString(f: Factor[Double], variableOrdering: Map[Int, Int]): String = {
    ...
    val indicesOrdering = new Ordering[List[Int]] { def compare(list1: List[Int], list2: List[Int]) = compareLists(list1, list2, thisFactorOrdering) }
  ...
  def ConvertToUAIString(): (String, String,Map[Int, Int] ) = {
    ...
    problemString.append(Properties.lineSeparator)
    allFactors.keys.map(k => problemString.append(getFactorInformation(allFactors(k), variablesToAbsoluteOrderPlacement)))
    allFactors.keys.map(k => problemString.append(allFactors(k).allIndices.size + Properties.lineSeparator + getUAIFactorString(allFactors(k), variablesToAbsoluteOrderPlacement) + Properties.lineSeparator))

    val evidenceString = convertEvidenceToUCIFormat(targetElements,variablesToAbsoluteOrderPlacement)

    (problemString.toString, evidenceString.toString(),variablesToAbsoluteOrderPlacement)
