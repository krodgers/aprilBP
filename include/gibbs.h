  ...
  void init() {
    ...
    if (!NoBest) {
      ...
      for (size_t f=0;f<nFactors();++f) _lb+=std::log( factor(f)[sub2ind(factor(f).vars(),_state)] );
		...
		temp = tempMin;	// !!! TODO:FIX temp definition reversed?
		//std::cout<<"Gibbs: t="<<temp<<"; noBest? "<<NoBest<<" noBeliefs? "<<NoBeliefs<<"; lb="<<_lb<<"\n";
  }
  ...
  void run() {
    ...
    for (size_t j=0,i=0; i<nSamples; ++i) {                          // keep nSamples evenly space samples
      ...
      for (;j<jNext;++j) {
        ...
        if (!NoBest && (isnan(score) || score==-infty())) {    // if we're still looking for a valid config
