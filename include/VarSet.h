...
template<>
struct mexable_traits<mex::Var> {
  typedef size_t   base;
  typedef size_t   mxbase;
...
//#ifdef MEX
//    if (mxAvail() && size()>1) mxSetN(mxGet(),v_.size());
//#endif
    return *this;
  }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  double         nrStatesDouble() const { double ns=1; for (size_t i=0;i<size();++i) ns*=d_[i]; return (ns==0)?1:ns; }
...
// Operators on single-element arguments
  ...
  /*
  // Alternate version that uses pointers instead of array references -- faster? (apparently not)
  ...
  */
  ...
  bool intersects(const VarSet& S) const { return (*this & S).size() > 0; }  // !!! faster
...
inline void ind2sub(const VarSet& vs, size_t i, MapType& val) {
  ...
  //return val;  // set value by reference instead
}
