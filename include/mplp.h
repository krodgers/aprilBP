...
public:
  ...
  virtual void setProperties(std::string opt=std::string()) {
	...
	void setStopIter(double v) { _stopIter = v; }
	void setStopObj(double v)  { _stopObj = v; }
	void setStopMsg(double v)  { _stopMsg = v; }
	void setStopTime(double v) { _stopTime = v; }
  ...
  void run() {
    ...
    if (iter>print*nFactors()) { 
			mex::graphModel tmp(beliefs());
			//vector<uint32_t> config = tmp.maxSequential(order(mex::graphModel::OrderMethod::Random));
			vector<uint32_t> config = maxSequentialFast(order(mex::graphModel::OrderMethod::Random));
			if (_LB < logP(config)) { _best = vector<index>(config.begin(),config.end()); _LB=logP(config); }
		}
    if (iter>print*nFactors()) { print++; std::cout<<"LB: "<<_LB<<", UB: "<<_UB<<"; d="<<dObj<<"\n"; }
